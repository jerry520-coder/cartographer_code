...
# 用来启动一个名为cartographer_assets_writer的节点，该节点可以从传感器数据和位姿图中生成地图资源，如点云、X-Rays或ROS地图。
...
def generate_launch_description():
    ...
    config_file_arg = DeclareLaunchArgument(
        "config_file", default_value="assets_writer_backpack_2d.lua"
    )
    ...
    cartographer_node = Node(
        ...
        arguments=[
            ...
            "-bag_filenames",
            LaunchConfiguration("bag_filenames"),
            "-pose_graph_filename",
            LaunchConfiguration("pose_graph_filename"),
        ],
        output="screen",
    )
